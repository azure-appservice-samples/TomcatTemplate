...
}ORYX.Plugins.ResourcesSoDShow=Clazz.extend({facade:undefined,construct:function(a){this.facade=a;
this.active=false;
this.raisedOverlayEventIds=[];
this.raisedHighlightEventIds=[];
this.facade.offer({name:ORYX.I18N.ResourcesSoDShow.name,functionality:this.showSoD.bind(this),group:ORYX.I18N.ResourcesSoDShow.group,dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/sod.png",icon:ORYX.BASE_FILE_PATH+"images/sod_view.png",description:ORYX.I18N.ResourcesSoDShow.desc,index:2,toggle:true,minShape:1,maxShape:1})
},showSoD:function(){this.removeHighlightsAndOverlays();
...
}else{alert("Please select a task to show the related Separation of Duties constraints.")
}},highlightSelectedTask:function(a){if(!(a instanceof ORYX.Core.Shape)){return
}this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_HIGHLIGHT_SHOW,highlightId:a.properties["oryx-id"],elements:[a],color:"#FF0000"});
this.raisedHighlightEventIds.push(a.properties["oryx-id"])
},showOverlaysForSeparations:function(c,g){if(!(c instanceof ORYX.Core.Shape)){return
}var h={fill:"#FF0000",stroke:"white","stroke-width":1};
...
e=0;
break;
case 8:alert("There exist more Separation of Duties constraints for task "+c.properties["oryx-id"]+", but they will not be illustrated by a number in that task's rectangle.");
break;
default:break
}if(g>=10){j=j-4
...
},prepareOverlays:function(c){if(c.properties["oryx-separationofduties"]!=""){var e=c.properties["oryx-separationofduties"].evalJSON();
var b=e.items.toArray();
var a;
for(var d=0;
d<b.length;
d++){var f=b[d].SeparatedTasks;
...
this.showOverlaysForSeparations(a,d+1)
}}else{alert("No Separation of Duties Constraints are defined for this task")
