...
if(c=Hash.toQueryString(c)){if(this.method=="get"||this.options.postBody){this.url+=(this.url.include("?")?"&":"?")+c
...
(this.options["on"+this.transport.status]||this.options["on"+(this.success()?"Success":"Failure")]||Prototype.emptyFunction)(g,b)
}catch(d){this.dispatchException(d)
}var f=this.getHeader("Content-type");
if(f&&f.strip().match(/^(text|application)\/(x-)?(java|ecma)script(;.*)?$/i)){this.evalResponse()
}}try{(this.options["on"+c]||Prototype.emptyFunction)(g,b);
...
}Element.extend=function(e){var f=Prototype.BrowserFeatures;
if(!e||!e.tagName||e.nodeType==3||e._extended||f.SpecificElementExtensions||e==window){return e
}var b={},d=e.tagName,a=Element.extend.cache,c=Element.Methods.ByTag;
if(!f.ElementExtensions){Object.extend(b,Element.Methods),Object.extend(b,Element.Methods.Simulated)
}if(c[d]){Object.extend(b,c[d])
}for(var h in b){var g=b[h];
if(typeof g=="function"&&!(h in e)){e[h]=a.findOrStore(g)
...
for(var c in g){if(a=f.match(g[c])){this.matcher.push(typeof b[c]=="function"?b[c](a):new Template(b[c]).evaluate(a));
f=f.replace(a[0],"");
break
...
for(var d in h){if(b=j.match(h[d])){c=typeof a[d]=="function"?a[d](b):new Template(a[d]).evaluate(b);
...
},descendant:'c = "descendant";',child:'c = "child";',adjacent:'c = "adjacent";',laterSibling:'c = "laterSibling";'},patterns:{laterSibling:/^\s*~\s*/,child:/^\s*>\s*/,adjacent:/^\s*\+\s*/,descendant:/^\s/,tagName:/^\s*(\*|[\w\-]+)(\b|$)?/,id:/^#([\w\-\*]+)(\b|$)/,className:/^\.([\w\-\*]+)(\b|$)/,pseudo:/^:((first|last|nth|nth-last|only)(-child|-of-type)|empty|checked|(en|dis)abled|not)(\((.*?)\))?(\b|$|\s|(?=:))/,attrPresence:/^\[([\w]+)\]/,attr:/\[((?:[\w-]*:)?[\w-]+)\s*(?:([!^$*~|]?=)\s*((['"])([^\]]*?)\4|([^'"][^\]]*?)))?\]/},handlers:{concat:function(d,c){for(var e=0,f;
f=c[e];
e++){d.push(f)
}return d
...
e++){for(var b=0,c=[],k;
k=g.childNodes[b];
b++){if(k.nodeType==1&&k.tagName!="!"){d.push(k)
}}}return d
...
},tagName:function(b,a,e,j){e=e.toUpperCase();
var d=[],f=Selector.handlers;
if(b){if(j){if(j=="descendant"){for(var c=0,g;
g=b[c];
...
},attr:function(a,h,g,j,b){if(!a){a=h.getElementsByTagName("*")
...
for(var o=0,f;
f=c[o];
o++){if(!f.parentNode._counted){q.index(f.parentNode,r,e);
d.push(f.parentNode)
...
},findChildElements:function(e,g){var j=g.join(","),g=[];
j.scan(/(([\w#:.~>+()\s-]+|\*|\[.*?\])+)\s*(,|$)/,function(h){g.push(h[1].strip())
});
var d=[],f=Selector.handlers;
