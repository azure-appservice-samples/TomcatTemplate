Ext.DomHelper=function(){var m=null;
...
var j=function(u,r,q,t){m.innerHTML=[r,q,t].join("");
var o=-1,p=m;
while(++o<u){p=p.firstChild
}return p
};
var k="<table>",e="</table>",c=k+"<tbody>",l="</tbody>"+e,i=c+"<tr>",d="</tr>"+l;
...
},applyStyles:function(q,r){if(r){q=Ext.fly(q);
...
}}else{if(typeof r=="function"){Ext.DomHelper.applyStyles(q,r.call())
}}}}},insertHtml:function(q,s,r){q=q.toLowerCase();
if(s.insertAdjacentHTML){if(b.test(s.tagName)){var p;
...
var d=function(h,k,o,j){if(o&&a){if(o.substr(0,5)=="this."){return c.call(o.substr(5),b[k],b)
}else{if(j){var n=/^\s*['"](.*)["']\s*$/;
j=j.split(",");
for(var l=0,g=j.length;
l<g;
l++){j[l]=j[l].replace(n,"$1")
}j=[b[k]].concat(j)
}else{j=[b[k]]
}return e[o].apply(e,j)
}}else{return b[k]!==undefined?b[k]:""
}};
...
var c,b,d;
var g=this;
var a=function(k,j,e){if((b=k.apply(j||g,e))!==undefined){if(typeof b==="object"){if(b.returnValue!==undefined){c=b.returnValue
}else{c=b
...
for(var k=0,e=h.before.length;
k<e;
k++){a(h.before[k].fn,h.before[k].scope,j);
if(d){return c
}}if((b=h.originalFn.apply(g,j))!==undefined){c=b
}for(var k=0,e=h.after.length;
k<e;
k++){a(h.after[k].fn,h.after[k].scope,j);
if(d){return c
}}return c
}
}return h
...
var c=function(i,j,g,d){return function(){j.removeListener(g,d);
...
}}},createListener:function(i,g,j){j=j||{};
g=g||this.obj;
var d={fn:i,scope:g,options:j};
var e=i;
if(j.delay){e=a(e,j,g)
}if(j.single){e=c(e,this,i,g)
}if(j.buffer){e=b(e,j,g)
}d.fireFn=e;
return d
},findListener:function(k,j){j=j||this.obj;
var g=this.listeners;
for(var h=0,d=g.length;
h<d;
h++){var e=g[h];
if(e.fn==k&&e.scope==j){return h
}}return -1
...
},fire:function(){var g=this.listeners,k,d=g.length;
...
Ext.EventManager=function(){var s,l,h=false;
var j,r,c,n;
var k=Ext.lib.Event;
var m=Ext.lib.Dom;
...
var a=function(){s=new Ext.util.Event();
...
t.onreadystatechange=function(){if(this.readyState=="complete"){b()
}}
}else{if(Ext.isSafari){l=setInterval(function(){var u=document.readyState;
if(u=="complete"){b()
}},10)
}}}k.on(window,"load",b)
};
var q=function(u,v){var t=new Ext.util.DelayedTask(u);
return function(w){w=new Ext.EventObjectImpl(w);
t.delay(v.buffer,u,null,[w])
}
};
var o=function(w,v,t,u){return function(x){Ext.EventManager.removeListener(v,t,u);
w(x)
}
};
var d=function(t,u){return function(v){v=new Ext.EventObjectImpl(v);
setTimeout(function(){t(v)
},u.delay||10)
}
};
var i=function(v,u,t,z,y){var A=(!t||typeof t=="boolean")?{}:t;
z=z||A.fn;
y=y||A.scope;
var x=Ext.getDom(v);
if(!x){throw'Error listening for "'+u+'". Element "'+v+"\" doesn't exist."
}var w=function(C){C=Ext.EventObject.setEvent(C);
var B;
if(A.delegate){B=C.getTarget(A.delegate,x);
if(!B){return
}}else{B=C.target
}if(A.stopEvent===true){C.stopEvent()
}if(A.preventDefault===true){C.preventDefault()
}if(A.stopPropagation===true){C.stopPropagation()
}if(A.normalized===false){C=C.browserEvent
}z.call(y||x,C,B,A)
};
if(A.delay){w=d(w,A)
}if(A.single){w=o(w,x,u,z)
}if(A.buffer){w=q(w,A)
}z._handlers=z._handlers||[];
z._handlers.push([Ext.id(x),u,w]);
k.on(x,u,w);
if(u=="mousewheel"&&x.addEventListener){x.addEventListener("DOMMouseScroll",w,false);
k.on(window,"unload",function(){x.removeEventListener("DOMMouseScroll",w,false)
})
}if(u=="mousedown"&&x==document){Ext.EventManager.stoppedMouseDownEvent.addListener(w)
}return w
};
var e=function(u,v,A){var t=Ext.id(u),B=A._handlers,y=A;
if(B){for(var w=0,z=B.length;
w<z;
w++){var x=B[w];
if(x[0]==t&&x[1]==v){y=x[2];
B.splice(w,1);
break
}}}k.un(u,v,y);
u=Ext.getDom(u);
if(v=="mousewheel"&&u.addEventListener){u.removeEventListener("DOMMouseScroll",y,false)
}if(v=="mousedown"&&u==document){Ext.EventManager.stoppedMouseDownEvent.removeListener(y)
}};
var g=/^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/;
var p={addListener:function(v,t,x,w,u){if(typeof t=="object"){var z=t;
for(var y in z){if(g.test(y)){continue
}if(typeof z[y]=="function"){i(v,y,z,z[y],z.scope)
}else{i(v,y,z[y])
}}return
}return i(v,t,u,x,w)
},removeListener:function(u,t,v){return e(u,t,v)
},onDocumentReady:function(v,u,t){if(h){s.addListener(v,u,t);
s.fire();
s.clearListeners();
return
}if(!s){a()
}s.addListener(v,u,t)
},onWindowResize:function(v,u,t){if(!j){j=new Ext.util.Event();
r=new Ext.util.DelayedTask(function(){j.fire(m.getViewWidth(),m.getViewHeight())
});
k.on(window,"resize",this.fireWindowResize,this)
}j.addListener(v,u,t)
},fireWindowResize:function(){if(j){if((Ext.isIE||Ext.isAir)&&r){r.delay(50)
}else{j.fire(m.getViewWidth(),m.getViewHeight())
}}},onTextResize:function(w,v,t){if(!c){c=new Ext.util.Event();
var u=new Ext.Element(document.createElement("div"));
u.dom.className="x-text-resize";
u.dom.innerHTML="X";
u.appendTo(document.body);
n=u.dom.offsetHeight;
setInterval(function(){if(u.dom.offsetHeight!=n){c.fire(n,n=u.dom.offsetHeight)
}},this.textResizeInterval)
}c.addListener(w,v,t)
},removeResizeListener:function(u,t){if(j){j.removeListener(u,t)
}},fireResize:function(){if(j){j.fire(m.getViewWidth(),m.getViewHeight())
}},ieDeferSrc:false,textResizeInterval:50};
p.on=p.addListener;
p.un=p.removeListener;
p.stoppedMouseDownEvent=new Ext.util.Event();
return p
}();
...
switch(a.toLowerCase()){case"t":g.setSize(j.width,0);
l.left=l.bottom="0";
k={height:h};
break;
...
g.show();
arguments.callee.anim=g.fxanim(k,c,"motion",0.5,"easeOut",e)
});
return this
...
g.position="absolute";
h.setSize(d);
var k=function(){if(c.useDisplay){b.setDisplayed(false)
}else{b.hide()
...
break
}arguments.callee.anim=h.fxanim(e,c,"motion",0.5,"easeOut",k)
});
return this
...
var j=function(){b.dom.style[d]=i;
b.afterFx(c)
};
var e={};
e[d]={from:a,to:g};
arguments.callee.anim=this.fxanim(e,c,"color",1,"easeIn",j)
});
return this
...
i.animate({top:{from:e.y,to:e.y-20},left:{from:e.x,to:e.x-20},borderWidth:{from:0,to:10},opacity:{from:1,to:0},height:{from:e.height,to:(e.height+(20*j))},width:{from:e.width,to:(e.width+(20*j))}},h,function(){i.remove();
if(--c>0){g()
}else{b.afterFx(d)
}})
};
...
b=b||{};
a.queueFx(b,function(){var e={},d=b.width,g=b.height,c=b.x,j=b.y,i=b.opacity;
if(d!==undefined){e.width={to:this.adjustWidth(d)}
}if(g!==undefined){e.height={to:this.adjustHeight(g)}
}if(c!==undefined||j!==undefined){e.points={to:[c!==undefined?c:this.getX(),j!==undefined?j:this.getY()]}
...
var d={opacity:{to:0},points:{}},k=d.points;
switch(a.toLowerCase()){case"t":k.by=[0,-i];
break;
case"l":k.by=[-e,0];
break;
case"r":k.by=[e,0];
...
case"tr":k.by=[e,-i];
break
}arguments.callee.anim=this.fxanim(d,c,"motion",0.5,"easeOut",l)
});
return this
...
if(e.isUpload||(j&&j.toLowerCase()=="multipart/form-data")){return this.doFormUpload(e,c,b)
}var i=Ext.lib.Ajax.serializeForm(d);
c=c?(c+"&"+i):i
}var k=e.headers;
if(this.defaultHeaders){k=Ext.apply(k||{},this.defaultHeaders);
...
for(var j in a){if(a.hasOwnProperty(j)){m=document.createElement("input");
m.type="hidden";
m.name=j;
m.value=a[j];
d.appendChild(m);
...
d.submit();
if(p){for(var l=0,n=p.length;
l<n;
l++){Ext.removeNode(p[l])
}}}});
...
this.delay=function(j,l,k,d){if(i&&j!=h){this.cancel()
}h=j;
b=new Date().getTime();
g=l||g;
e=k||e;
